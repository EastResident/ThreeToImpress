...
window.three_app_controls = new Three_app
...
window.onmousedown = (e) ->
	...
	projector.unprojectVector(vector, window.three_app_controls.camera)
	ray = new THREE.Raycaster(window.three_app_controls.camera.position, vector.sub(window.three_app_controls.camera.position).normalize())

	count = 0
	for object in @three_object
		if(ray.intersectObject(object).length > 0)
			window.three_app_controls.transctrl.detach()
			window.three_app_controls.transctrl.attach(@three_object[count])

		count += 1

@set_translate = ->
	window.three_app_controls.transctrl.setMode("translate")
@set_rotate = ->
	window.three_app_controls.transctrl.setMode("rotate")
@set_scale = ->
	window.three_app_controls.transctrl.setMode("scale")
...
@arrangement = (_slide_object) ->
	...
	window.three_app_controls.scene.add(@three_object[@slide_count])

	# window.three_app_controls.transctrl.attach(@three_object[@slide_count])
  # t.transctrl.update()
...
@renderloop = ->
	...
	window.three_app_controls.controls.update()
	window.three_app_controls.renderer.render( window.three_app_controls.scene, window.three_app_controls.camera )
